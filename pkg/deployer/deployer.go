...
func NewAWSRunner(d deployer, cfg remote.Config) *AWSRunner {
...
func (a *AWSRunner) prepareAWSImages() ([]internalAWSImage, error) {
	...
	if a.cfg.ImageConfigFile != "" {
		...
		for shortName, imageConfig := range externalImageConfig.Images {
			...
			if a.deployer.InstanceProfile != "" && imageConfig.InstanceProfile != "" {
			...
			if awsImage.instanceType == "" {
				awsImage.instanceType = a.deployer.InstanceType
			}
...
//	}
//	if instance.sshPublicKeyFile != "" && a.deployer.Ec2InstanceConnect {
...
func (a *AWSRunner) getAWSInstance(img internalAWSImage) (*awsInstance, error) {
	...
	//if *a.reuseInstances && len(existing.Reservations) > 0 && len(existing.Reservations[0].Instances) > 0 {
